...
implemented as a first backend persistence foundation for metadata-only
checks. Runtime Gateway behavior, PolicyRule schemas, scanner integration,
public CRUD APIs, frontend UI, and external integrations have not changed yet.
...
metadata. They are not currently generated by Runtime Gateway or included in
