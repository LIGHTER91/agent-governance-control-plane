...
def main() -> int:
    parser = ArgumentParser(
        description=(
            "Seed DB-backed service actor scope and fine-grained rule records "
            "from AGCP_SERVICE_ACTOR_SCOPES and AGCP_SERVICE_ACTOR_SCOPE_RULES."
        )
    )
    parser.add_argument(
        "--apply",
    ...
    try:
        with Session(engine) as session:
            ...
            if dry_run:
                session.rollback()
            print(format_scope_seed_results(results, dry_run=dry_run))
    finally:
        engine.dispose()
